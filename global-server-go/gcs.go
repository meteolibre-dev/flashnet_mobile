...
// gdalUseVSICurl is set when no GCS credentials are available: GDAL then
// reads the (public) bucket over plain HTTPS via /vsicurl/ instead of the
// authenticated /vsigs/ filesystem.
var gdalUseVSICurl = false

// buildVSUrl returns a GDAL virtual-file URL for a bucket-relative path.
func buildVSUrl(bucket, path string) string {
	if gdalUseVSICurl {
		return fmt.Sprintf("/vsicurl/https://storage.googleapis.com/%s/%s", bucket, path)
	}
	return fmt.Sprintf("/vsigs/%s/%s", bucket, path)
}
...
func getGCSService() *storagev1.Service {
	gcsServiceOnce.Do(func() {
		...
		useAnonymous := func() bool {
			if v := os.Getenv("GCS_ANONYMOUS"); v == "1" || v == "true" {
				return true
			}
			if _, err := google.FindDefaultCredentials(ctx); err != nil {
				return true // no credentials at all — public buckets still work
			}
			return false
		}
		var svc *storagev1.Service
		var err error
		if useAnonymous() {
			log.Println("[INIT] Using anonymous GCS access (public bucket)")
			svc, err = storagev1.NewService(ctx, option.WithoutAuthentication())
		} else {
...
// forecast_{YYYYMMDDHHMM}_{fileband}.tif — global model uses .tif extension.
...
func getCOGUrl(timestamp, band, runTime string) string {
	...
	fileBand := "sat"
	if cfg, ok := BANDS[band]; ok {
		fileBand = cfg.FileBand
	}

	if runTime != "" {
		runDate := extractRunDate(runTime)
		return buildVSUrl(bucket, fmt.Sprintf("forecasts/%s/%s/forecast_%s_%s.tif",
			runDate, runTime, timestamp, fileBand))
	}
	...
	if h5Sub, ok := findH5Subfolder(timestamp); ok {
		...
		return buildVSUrl(bucket, fmt.Sprintf("forecasts/%s/%s/forecast_%s_%s.tif",
			runDate, h5Sub, timestamp, fileBand))
	}

	// Fallback to flat layout
	dateFolder := timestamp[:8]
	return buildVSUrl(bucket, fmt.Sprintf("forecasts/%s/forecast_%s_%s.tif",
		dateFolder, timestamp, fileBand))
}

// findH5Subfolder finds the run subfolder (YYYYMMDD_HHMM) for a given forecast
// timestamp. Searches the timestamp's date folder and the two previous days.
func findH5Subfolder(timestamp string) (string, bool) {
	...
	// NOTE: this bucket uses compact date folders (forecasts/YYYYMMDD/).
	for offset := 0; offset < 3; offset++ {
		dateFolder := tsDt.AddDate(0, 0, -offset).Format("20060102")
...
func verifyCogFileReady(timestamp, band string) bool {
	...
	if h5Sub, ok := findH5Subfolder(timestamp); ok {
		...
		dateFolder := timestamp[:8]
		blobName = fmt.Sprintf("forecasts/%s/forecast_%s_%s.tif", dateFolder, timestamp, fileBand)
	}
...
// fileBandFilter resolves the `band` query parameter (logical band name, a
// file band suffix, or "any") to the file band suffix used to filter files.
func fileBandFilter(band string) string {
	if band == "any" || band == "" {
		return "any"
	}
	if cfg, ok := BANDS[band]; ok {
	...
	return band // treat as a raw file band suffix
}
...
func handleAvailable(ctx context.Context, days int, band string) (*AvailableResponse, error) {
	...
	for i := -1; i < days; i++ {
		d := now.AddDate(0, 0, i)
		// Compact date folder: forecasts/YYYYMMDD/
		dateFolder := d.Format("20060102")
	...
	timestampBands := make(map[string]map[string]bool) // timestamp → set of file bands
	...
	for _, ts := range timestamps {
		bands := logicalBandsFor(timestampBands[ts])
		if len(bands) == 0 {
			continue
		}
		...
		if latestSub != "" {
			runDate := extractRunDate(latestSub)
			tiffURL = fmt.Sprintf("https://storage.googleapis.com/%s/forecasts/%s/%s/forecast_%s_{band}.tif",
				bucket, runDate, latestSub, ts)
		} else {
			tiffURL = fmt.Sprintf("https://storage.googleapis.com/%s/forecasts/%s/forecast_%s_{band}.tif",
				bucket, ts[:8], ts)
		}
...
// logicalBandsFor maps a set of file bands found in the bucket to the logical
// bands the server can serve from them.
func logicalBandsFor(fileBands map[string]bool) []string {
	set := make(map[string]bool)
	for fb := range fileBands {
		for _, logical := range fileBandToLogical[fb] {
			set[logical] = true
		}
	}
...
func handleHistoryDateRuns(ctx context.Context, date, band string) (*HistoryDateResponse, error) {
	...
	// NOTE: the bucket uses compact date folders (forecasts/YYYYMMDD/).
	var scanDates []string
	for offset := 0; offset < 3; offset++ {
		scanDates = append(scanDates, reqDt.AddDate(0, 0, -offset).Format("20060102"))
	}

	filter := fileBandFilter(band)
	...
	for _, scanDate := range scanDates {
		...
		if len(resp.Prefixes) == 0 {
			// Flat layout
			runTimestamps, allBands := scanRunTimestamps(ctx, svc, bucket, datePrefix, scanDate, filter, "", scanDate)
			...
			for _, sub := range subs {
				...
				runTimestamps, allBands := scanRunTimestamps(ctx, svc, bucket, runPrefix, scanDate, filter, sub, scanDate)
				if len(runTimestamps) > 0 {
					runs = append(runs, HistoryRun{
						RunTime:        sub,
						DateFolder:     scanDate,
...
// scanRunTimestamps lists files under a prefix and extracts timestamps for the
// requested date. `filter` is a file band suffix ("sat") or "any".
// `reqDate` is a compact date (YYYYMMDD).
func scanRunTimestamps(ctx context.Context, svc *storagev1.Service, bucket, prefix, reqDate, filter, runTime, scanDate string) ([]HistoryRunTimestamp, []string) {
	resp, err := svc.Objects.List(bucket).Prefix(prefix).MaxResults(1000).Context(ctx).Do()
	if err != nil {
		return nil, nil
	}
	...
	for _, obj := range resp.Items {
		...
		// Only include timestamps on the requested date (compact YYYYMMDD)
		if ts[:8] != reqDate {
			continue
		}
		if filter != "any" && foundBand != filter {
			continue
		}
		if tsBands[ts] == nil {
	...
	for _, ts := range tsList {
		...
		bands := logicalBandsFor(tsBands[ts])
		for _, b := range bands {
			allBandsSet[b] = true
		}
...
func setupGDALGCSAuth() {
	...
	// No credentials anywhere — fall back to anonymous /vsicurl/ access.
	// Works for buckets with public (allUsers) read access.
	gdalUseVSICurl = true
	log.Println("[INIT] No GCS credentials found — using anonymous /vsicurl/ access (public bucket)")
}
