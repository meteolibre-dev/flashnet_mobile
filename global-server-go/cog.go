...
// ----------------------------------------------------------------------------
// Unlike lightning-server-go, the global model stores multiple raster bands in
// a single COG (forecast_{ts}_sat.tif: band 1 = VIS, band 2 = IR), so every
// read function takes a 1-based bandIndex parameter.
// ============================================================================
...
// getCOGInfo reads metadata from a COG for the given 1-based raster band.
func getCOGInfo(url string, bandIndex int) (*COGInfo, error) {
...
func readTile(url string, bandIndex, z, x, y, tileSize int) ([]float32, *float64, error) {
	ds, err := cogPool.Open(url)
	if err != nil {
		return nil, nil, err
	}
	...
	band := ds.Band(bandIndex)
	if band == nil {
		return nil, nil, fmt.Errorf("no band %d in %s", bandIndex, url)
	}
...
func readPreview(url string, bandIndex, targetW, targetH int) ([]float32, int, int, *float64, error) {
	ds, err := cogPool.Open(url)
	if err != nil {
	...
	band := ds.Band(bandIndex)
	if band == nil {
		return nil, 0, 0, nil, fmt.Errorf("no band %d", bandIndex)
	}
...
func readPoint(url string, bandIndex int, lat, lon float64) (float64, error) {
	ds, err := cogPool.Open(url)
	if err != nil {
		return 0, err
	}
	...
	band := ds.Band(bandIndex)
	if band == nil {
		return 0, fmt.Errorf("no band %d", bandIndex)
	}
